@echo off
REM 🚀 AI Financial Coach Deployment Script for Windows

echo 🎯 Preparing AI Financial Coach for Deployment

REM Clean up any existing git issues
echo 📝 Cleaning Git cache...
cd /d "%~dp0"

REM Remove node_modules from git if tracked
git rm -r --cached professional-frontend/node_modules/ 2>nul || echo node_modules not in git

REM Configure git for Windows long paths
